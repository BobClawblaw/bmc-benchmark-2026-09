...
| gate: "muhash parity BLOCKED under store load" (was: PASS on quiesced store) | none yet — TXOQ-1 (gettxout "not ready" loop) and RPC-1 (handler deadline) are still open sketches; the other session's rpc_server.c change may cover RPC-1 | unchanged: parity itself is PASS (three-way, strict rule); the BLOCKED note describes the loaded-RPC condition, which stands until TXOQ-1/RPC-1 land |
